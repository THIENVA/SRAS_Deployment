...
const EditSkillModal = ({ open, handleClose, userId, skills, setSkills, getSkill }) => {
    const { updateSkill } = useProfile()
    const [loading, setLoading] = useState(false)
    const showSnackbar = useSnackbar()
    const [scholarship, setScholarship] = useState({
        name: getSkill.name,
        description: getSkill.description,
        result: getSkill.result,
        issuer: getSkill.issuer,
    })
    ...
    const [issuedYear, setIssuedYear] = useState(getSkill.issuedYear ? new Date(getSkill.issuedYear, 0) : null)

    const { name, description, issuer, result } = scholarship
    ...
    const handleSubmit = () => {
        ...
        if (!isName) {
            ...
            const formatSkill = cloneDeep(skillData)
            const cloneSkills = cloneDeep(skills)
            const position = cloneSkills.findIndex((item) => item.id === getSkill.id)
            cloneSkills.splice(position, 1, formatSkill)
            updateSkill(userId, cloneSkills)
                .then(() => {
        ...
        >
            <Box sx={{ border: '2px solid #000' }}>
                ...
                <DialogContent dividers>
                    ...
                    <InputLayout isRequired={false} boxStyle={{ mb: 2 }} label="Score / Result">
                        <TextField size="small" fullWidth value={result} name="result" onChange={handleScholarship} />
                    </InputLayout>
