...
const OptionValueSection = ({ showAs, setShowAs }) => {
    const { result } = showAs

    const handleSelectedOption = (event) => {
        const { value } = event.target
        const updatedShowAs = cloneDeep(showAs)
        if (value === SHOW_AS.RADIO) {
            updatedShowAs.result.multiple = false
            updatedShowAs.result.valueInput = ''
            updatedShowAs.result.valueInputType = 'number'
        } else if (value === SHOW_AS.SELECT_ONE) {
            updatedShowAs.result.multiple = false
            updatedShowAs.result.valueInput = { value: '', point: '' }
            updatedShowAs.result.valueInputType = 'number'
    ...
    const handleAddingMoreChoice = () => {
        const newChoice = { id: uuid(), value: '', point: 1 }
        const updatedShowAs = cloneDeep(showAs)
    ...
    const handlePointChange = (event, index) => {
        const updatedShowAs = cloneDeep(showAs)
        updatedShowAs.result.value[index].point = event.target.value
        setShowAs(updatedShowAs)
    }
...
export default memo(OptionValueSection)
