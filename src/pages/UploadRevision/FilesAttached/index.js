            ...
            <Box mt={4} mb={3}>
                ...
                <Box mt={2}>
                    <Typography variant="body2" sx={{ color: AppStyles.colors['#ffa500'] }}>
                        {config
                            ? `You must upload at least ${config.revisionSettings[2].value} file(s) `
                            : 'You may not have to upload file'}
                        {config ? `up to ${config.revisionSettings[1].value} file(s) ` : ''}. Maximum file size is{' '}
                        {config ? config.revisionSettings[3].value : 'No limit '}
                        Mb. We accept{' '}
                        <strong>
                            {' '}
                            {config
                                ? config.revisionSettings[0].value.length > 0 &&
                                  config.revisionSettings[0].value.map((item) => `${item}, `)
                                : 'doc, docx, pdf'}
                        </strong>{' '}
                        formats.
                    </Typography>
                    ...
                    <Box component="form" ref={ref}>
                        {' '}
                        <InputLabel
                            disabled={config ? filesAttached.length === config.revisionSettings[1].value : false}
                            htmlFor="upload"
