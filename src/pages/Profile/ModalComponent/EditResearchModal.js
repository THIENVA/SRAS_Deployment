...
const EditResearchModal = ({ open, handleClose, userId, researches, setResearches, getResearch }) => {
    const { updateResearch } = useProfile()
    const [loading, setLoading] = useState(false)
    const showSnackbar = useSnackbar()
    const [research, setScholarship] = useState({
        name: getResearch.researchDirectionName,
    })
    ...
    const handleSubmit = () => {
        ...
        if (!isName) {
            ...
            const position = cloneResearches.findIndex(
                (item) => item.researchDirectionId === getResearch.researchDirectionId
            )
            cloneResearches.splice(position, 1, formatResearch)
            updateResearch(userId, cloneResearches)
                .then(() => {
