...
const copyRightFile = ({ copyrightFile, setStateCopyrightFile }) => {
    const handleSubmissionFile = (event, index, isMultiple) => {
        const updatedStateSubmissionFile = cloneDeep(copyrightFile)
        if (isMultiple) {
            const {
                target: { value },
            } = event
        ...
        setStateCopyrightFile(updatedStateSubmissionFile)
    }
    return (
        <SectionLayout title={'COPYRIGHT FILE'}>
            {copyrightFile.map((submissionFile, index) => {
...
export default memo(copyRightFile)
