...
const SubmissionFile = ({ stateSubmissionFile, setStateSubmissionFile }) => {
    const handleSubmissionFile = (event, index, isMultiple) => {
        const updatedStateSubmissionFile = cloneDeep(stateSubmissionFile)
        if (isMultiple) {
            const {
                target: { value },
            } = event
        ...
        setStateSubmissionFile(updatedStateSubmissionFile)
    }
    return (
        <SectionLayout title={'SUBMISSION FILE'}>
            {stateSubmissionFile.map((submissionFile, index) => {
...
export default memo(SubmissionFile)
