...
const CameraReadyFile = ({ cameraReadyFile, setStateCameraReadyFile }) => {
    const handleSubmissionFile = (event, index, isMultiple) => {
        const updatedStateSubmissionFile = cloneDeep(cameraReadyFile)
        if (isMultiple) {
            const {
                target: { value },
            } = event
        ...
        setStateCameraReadyFile(updatedStateSubmissionFile)
    }
    return (
        <SectionLayout title={'CAMERA READY FILE'}>
            {cameraReadyFile.map((submissionFile, index) => {
...
export default memo(CameraReadyFile)
