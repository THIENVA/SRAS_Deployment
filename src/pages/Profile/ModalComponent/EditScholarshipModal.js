...
const EditScholarshipModal = ({ open, handleClose, userId, scholarships, setScholarships, getScholarship }) => {
    const { updateScholarship } = useProfile()
    const [loading, setLoading] = useState(false)
    const showSnackbar = useSnackbar()
    const [scholarship, setScholarship] = useState({
        name: getScholarship.name,
        description: getScholarship.description,
        issuer: getScholarship.issuer,
    })
    ...
    const [issuedYear, setIssuedYear] = useState(
        getScholarship.issuedYear ? new Date(getScholarship.issuedYear, 0) : getScholarship.issuedYear
    )
    ...
    const handleSubmit = () => {
        ...
        if (!isName) {
            ...
            const formatScholarship = cloneDeep(scholarshipData)
            const cloneScholarships = cloneDeep(scholarships)
            const position = cloneScholarships.findIndex((item) => item.id === getScholarship.id)
            cloneScholarships.splice(position, 1, formatScholarship)
            updateScholarship(userId, cloneScholarships)
                .then(() => {
                    setScholarships(cloneScholarships)
                    handleClose()
                })
                .catch(() => {
                    showSnackbar({
