...
const EditAwardModal = ({ open, handleClose, userId, awards, setAwards, getAward }) => {
    const { updateAward } = useProfile()
    const [loading, setLoading] = useState(false)
    const showSnackbar = useSnackbar()
    const [scholarship, setScholarship] = useState({
        name: getAward.name,
        description: getAward.description,
        issuer: getAward.issuer,
    })
    ...
    const [issuedYear, setIssuedYear] = useState(
        getAward.issuedYear ? new Date(getAward.issuedYear, 0) : getAward.issuedYear
    )
    ...
    const handleSubmit = () => {
        ...
        if (!isName) {
            ...
            const position = cloneAwards.findIndex((item) => item.id === getAward.id)
            cloneAwards.splice(position, 1, formatAward)
            updateAward(userId, cloneAwards)
                .then(() => {
