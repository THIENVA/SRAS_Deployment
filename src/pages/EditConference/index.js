...
const EditConference = () => {
    ...
    const { updateConference, notifyChairEmail, checkShortNameExisted, getConferenceInfo } = useConference()
    ...
    const [exShortName, setExShortName] = useState('')
    ...
    const submitConference = (event) => {
        ...
        if (!isValid) {
            ...
            if (image.file && image.src) {
                    ...
                    () => {
                        getDownloadURL(uploadTask.snapshot.ref)
                            .then((downloadURL) => {
                                ...
                                const conferenceUpdate = updateConference(conferenceId, conferenceInfo)
                                const notifyEmail = notifyChairEmail({
                                    emails: chairsEmail,
                                    conferenceName: fullName,
                                    shortNameConference: shortName,
                                })
                                Promise.all([conferenceUpdate, notifyEmail])
                                    .then(() => {
            ...
            } else if (image.file === null && image.src) {
                const formatDate = moment(startDate).format()
                const startDateFormat = formatDate.substring(0, 19)
                const formatEndDate = moment(endDate).format()
                const endDateFormat = formatEndDate.substring(0, 19)
                ...
                Promise.all([conferenceUpdate, notifyEmail])
                    .then(() => {
                        history.push('/conferences')
                        showSnackbar({ severity: 'success', children: 'Edit conference successfully' })
                    })
                    .catch(() => {
                        showSnackbar({
                            severity: 'error',
        ...
        if (conferenceId) {
            getConferenceInfo(conferenceId)
                .then((response) => {
                    ...
                    setExShortName(data.shortName)
                    setSelectField(data.country)
                    setFileName('Conference logo')
                    setDateField((prev) => ({
                        ...prev,
                        startDate: new Date(data.startDate),
                        endDate: new Date(data.endDate),
                    }))
                    setImage({ file: null, src: data.logo })
                    const formatChairs = data.chairs.map(({ accountId, ...rest }) => ({ id: accountId, ...rest }))
                    setUserTable(formatChairs)
                })
                .catch(() => {
                    // showSnackbar({
                    //     severity: 'error',
