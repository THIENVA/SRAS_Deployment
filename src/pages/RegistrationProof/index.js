...
import { REGISTER_FREE_PAPER, SUBMIT_PROOF_PAPER } from '~/constants/steps'
...
const RegistrationProof = (props) => {
    ...
    useEffect(() => {
        ...
        const registrablePapers = getRegistrationPapers(conferenceId, userIdProof, secondSignal)
        Promise.all([settings, registrablePapers])
            .then((response) => {
                ...
                if (response[1].data.registrablePapers) {
                    ...
                    setListDrop([{ submissionId: 0, submissionTitle: 'None' }, ...response[1].data.registrablePapers])
                } else {
                    showSnackbar({
                        severity: 'error',
                        children: 'User has no ready camera submission.',
                    })
                ...
                <Fragment>
                    ...
                    {(() => {
                        switch (activeStep) {
                            ...
                            case 1:
                                return <UploadProof order={order} handleBack={handleBack} handleNext={handleNext} />
                            case 2:
                                return <PaymentSuccess order={order} handleNext={handleNext} />
                            case 3:
                                return <RegistrationSummary order={order} userInfo={userDetail?.userInfo} />
                        }
                    })()}
                </Fragment>
            )}
