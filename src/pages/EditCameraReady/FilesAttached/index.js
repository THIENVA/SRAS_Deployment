        ...
        <Box mb={3}>
            ...
            <Box mt={2}>
                <Typography variant="body2" sx={{ color: AppStyles.colors['#ffa500'] }}>
                    {config
                        ? `You must upload at least ${config.cameraReadyFileSetting[1].value} file(s) `
                        : 'You may not have to upload file'}
                    {config ? `up to ${config.cameraReadyFileSetting[0].value} file(s) ` : ''}. Maximum file size is{' '}
                    {config ? config.cameraReadyFileSetting[2].value : 'No limit '}
                    Mb
                </Typography>
                ...
                <Box component="form" ref={ref}>
                    <InputLabel
                        disabled={config ? filesAttached.length === config.cameraReadyFileSetting[0].value : false}
                        htmlFor="upload"
