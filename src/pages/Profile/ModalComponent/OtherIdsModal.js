...
const OtherIdsModal = ({ open, handleClose, otherIds, profile, setProfile }) => {
    const { userId } = useAppSelector((state) => state.auth)
    const showSnackbar = useSnackbar()
    const { updateOtherIds } = useProfile()
    const [loading, setLoading] = useState(false)
    const [links, setLinks] = useState(otherIds)
    ...
    const handleSubmit = (event) => {
        ...
        const isInvalid = links.some(
            (link) => isEmpty(link.linkTitle) || isEmpty(link.linkUrl) || !regexUrl.test(link.linkUrl)
        )
        if (isInvalid) {
            const updatedLinks = links.map((link) => {
                if (isEmpty(link.linkTitle)) {
                    link.messageLinkTitle = 'The Id title must not be empty'
                }
                if (isEmpty(link.linkUrl)) {
                    link.messageLinkUrl = 'The Id link url must not be empty'
                } else if (!regexUrl.test(link.linkUrl)) {
            ...
            updateOtherIds(userId, data)
                .then(() => {
                    const cloneProfile = cloneDeep(profile)
                    cloneProfile.otherIds = JSON.stringify(links)
            ...
            <Box sx={{ border: '2px solid #000' }}>
                ...
                <DialogContent dividers>
                    <Typography variant="subtitle2">
                        Feel free to include any additional IDs from other Researcher Databases in the designated field
                        for other IDs.
                    </Typography>
